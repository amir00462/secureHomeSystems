...
@Composable
fun ChangePasswordInApp(typeIsModir: Boolean) {
    ...
    fun validateInputs(): Boolean {
        ...
        if (oldPassword.value == databaseService.readFromLocal(KEY_USER_PASSWORD)) {
            if (newPassword.value.length == 6 && newPasswordTekrar.value.length == 6) {
                if (newPassword.value == newPasswordTekrar.value) {
                    return true
                } else {
                    context.showToast("پسورد وارد شده با تکرار پسورد برابر نیست")
                }
            } else {
                context.showToast("پسورد باید حتما ۶ کاراکتر باشد")
                return false
            }
        } else {
            return false
        ...
        Surface(color = MaterialTheme.colors.background, modifier = Modifier.fillMaxSize()) {
            ...
            Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                ...
                Text(
                    ...
                    fontSize = 22.sp,
                    text = if(typeIsModir) "تغییر رمز ورود مدیران دستگاه" else "تغییر رمز ورود کاربران دستگاه",
                    modifier = Modifier.padding(top = 22.dp, start = 16.dp)
                )

                OtpTextField(
                    modifier = Modifier.padding(top = 24.dp),
                    txtSubject = "رمز ورود فعلی",
                    onValueChanged = { oldPassword.value = it },
